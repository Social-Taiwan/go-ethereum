...
func (ec *Client) BlockByNumberBase(ctx context.Context, number *big.Int) (*types.Block, error) {
	return ec.getBlockBase(ctx, "eth_getBlockByNumber", toBlockNumArg(number), true)
}
...
type rpcBlockBase struct {
	Hash         common.Hash          `json:"hash"`
	Transactions []rpcTransactionBase `json:"transactions"`
	UncleHashes  []common.Hash        `json:"uncles"`
	Withdrawals  []*types.Withdrawal  `json:"withdrawals,omitempty"`
}

func (ec *Client) getBlockBase(ctx context.Context, method string, args ...interface{}) (*types.Block, error) {
	...
	if transactions, ok := result["transactions"].([]interface{}); ok {
		...
		if len(transactions) > 0 {
			for _, tx := range transactions {
				if ok := tx.(map[string]interface{})["mint"] != nil; !ok {
					txs = append(txs, tx)
				}
			}
		...
		fmt.Println("transactions is not a slice or does not exist")
	}

	updatedBytes, err := json.Marshal(result)
	if err != nil {
		fmt.Println("Error marshaling:", err)
		return nil, err
	}

	if err := json.Unmarshal(updatedBytes, &body); err != nil {
		return nil, err
	}
...
type rpcTransactionBase struct {
	tx *types.TransactionBase
	txExtraInfo
}
